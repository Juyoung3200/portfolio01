$(document).ready(function () {
  // XX_feild_organ -> 대기데이터 목록 테이블

  $("#excelDataList").DataTable({
    searching: false,
    info: false,
...
function itemChange(e) {
  var daejeon = ["시/군/구 선택", "서구", "유성구"];
  var daegu = ["시/군/구 선택", "달서구", "수성구"];
  var busan = ["시/군/구 선택", "해운대구", "수영구"];
  var select13 = document.getElementById("select13");

  if (e.value == "대전") {
    var changeItem = daejeon;
  } else if (e.value == "대구") {
    var changeItem = daegu;
  } else if (e.value == "부산") {
    var changeItem = busan;
  }

  select13.options.length = 0;

  for (x in changeItem) {
    var opt = document.createElement("option");
    opt.value = changeItem[x];
    opt.innerHTML = changeItem[x];
    select13.appendChild(opt);
  }
}

function itemChange2(e) {

  var seogu = ["읍/면/동 선택", "둔산동", "월평동"];
  var yuseonggu = ["읍/면/동 선택", "봉명동", "장대동"];
  var dalseogu = ["읍/면/동 선택", "신당동", "장기동"];
  var suseonggu = ["읍/면/동 선택", "상동", "두산동"];
  var haeundaegu = ["읍/면/동 선택", "우동", "송정동"];
  var suyeonggu = ["읍/면/동 선택", "광안동", "민락동"];
  var select14 = document.getElementById("select14");

  if (e.value == "서구") {
    var changeItem = seogu;
  } else if (e.value == "유성구") {
    var changeItem = yuseonggu;
  } else if (e.value == "달서구") {
    var changeItem = dalseogu;
  } else if (e.value == "수성구") {
    var changeItem = suseonggu;
  } else if (e.value == "해운대구") {
    var changeItem = haeundaegu;
  } else if (e.value == "수영구") {
    var changeItem = suyeonggu;
  }
  select14.options.length = 0;

  for (x in changeItem) {
    var opt = document.createElement("option");
    opt.value = changeItem[x];
    opt.innerHTML = changeItem[x];
    select14.appendChild(opt);
  }
}